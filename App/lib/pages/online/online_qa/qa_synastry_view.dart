...
import 'qa_synastry_controller.dart';

class QaSynastryPage extends GetView<QaSynastryController> {
  const QaSynastryPage({super.key});

  @override
  Widget build(BuildContext context) {
    Get.lazyPut(() => QaSynastryController());
    final textStyle = TextStyle(fontSize: 14, color: Color(0xffA6A6A6));

    return Scaffold(
      appBar: AppBar(title: const Text('合盘解读'), backgroundColor: Colors.white),
            ...
            child: Column(
              ...
              children: [
                Container(
                  ...
                  padding: EdgeInsets.all(AppConst.PAGE_PADDING),
                  child: Obx(() {
                    final name1 = controller.archive1.value?.name ?? '未选择';
                    final name2 = controller.archive2.value?.name ?? '未选择';
                    return Text('$name1 & $name2', style: textStyle);
                  }),
                ),
